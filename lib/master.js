...
const { isObject, isFunction, isAsyncFunction } = require('./util');
...
async function childProcessMessageHandle(msg, handle) {
...
function startWorker(queueName, workerFile) {
  ...
  childProcess.send({
    ...
    type: MSG_TYPES.BIND_MSG_HANDLE,
  });
  childProcess.on('message', childProcessMessageHandle);
 ...
 */
function start(rabbitConfigure, queuesConf, options= null) {
  if (!rabbitConfigure || !queuesConf) {
    throw new Error('param is needed.')
  }
  rabbitConf = rabbitConfigure
  if (options && options.appName) {
    appName = options.appName.toString().trim();
  }
